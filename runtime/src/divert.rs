...
impl Divert {
    ...
    pub fn get_target_pointer(self: &Rc<Self>) -> Pointer {
            ...
            .is_index()
        {
            ...
            let container = target_obj.into_any().downcast::<Container>().unwrap();
...
mod tests {
    ...
    #[test]
    fn cached_target_does_not_retain_its_container() {
        ...
        let loop_container = Container::new(
            Some("loop".to_owned()),
            0,
            vec![divert.clone()],
            HashMap::new(),
        );
        let root = Container::new(None, 0, vec![loop_container.clone()], HashMap::new());
        let weak_loop = Rc::downgrade(&loop_container);

        let target = divert.get_target_pointer();
        assert!(Rc::ptr_eq(
        ...
        assert!(weak_loop.upgrade().is_none());
    }
}
